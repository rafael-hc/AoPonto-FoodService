...
const synchronizeWizardQuestionSchema = z.object({
  ...
  options: z.array(z.object({
    ...
    description: z.string().min(1, { error: 'A descrição da opção é obrigatória' }),
    promoPrice: z.number().optional().nullable(),
    maxQty: z.number().min(1)
  })).min(1, { error: 'A pergunta deve ter pelo menos uma opção' })
})

export class SynchronizeWizardQuestionDto extends createZodDto(synchronizeWizardQuestionSchema) {}
