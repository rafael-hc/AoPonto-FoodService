...
const DataDisplayField = ({ label, value, className = "" }: { label: string; value: string; className?: string }) => (
  <div className={`space-y-1 ${className}`}>
    <span className="text-[9px] font-black uppercase tracking-[0.1em] text-slate-400 ml-0.5">
      {label}
    </span>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="px-6 py-4 bg-slate-50/80 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                   ...
                   <h3 className="font-bold text-slate-800 text-sm">Dados da Assinatura</h3>
                </div>
                <button type="button" className="text-[10px] font-black uppercase tracking-widest text-orange-500 hover:text-orange-600 flex items-center gap-1.5 transition-colors">
                  Alterar <ExternalLink size={12} />
                </button>
              </div>

              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <DataDisplayField label="Nome Fantasia" value={MOCK_SUBSCRIPTION.fantasyName} />
                <DataDisplayField label="CNPJ" value={MOCK_SUBSCRIPTION.cnpj} />
                <DataDisplayField label="Razão Social" value={MOCK_SUBSCRIPTION.socialName} className="md:col-span-2" />
                
                <div className="md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-slate-50 pt-4 mt-2">
                   <DataDisplayField label="Endereço" value={MOCK_SUBSCRIPTION.address} className="col-span-2" />
                   <DataDisplayField label="Número" value={MOCK_SUBSCRIPTION.number} />
                   <DataDisplayField label="Cidade" value={MOCK_SUBSCRIPTION.city} />
                   <DataDisplayField label="Fone" value={MOCK_SUBSCRIPTION.phone} />
                   <DataDisplayField label="Estado" value={MOCK_SUBSCRIPTION.state} />
                   <DataDisplayField label="CEP" value={MOCK_SUBSCRIPTION.zipcode} className="col-span-2 md:col-span-1" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="px-6 py-4 bg-slate-900 border-b border-slate-800 flex items-center gap-2.5">
                 ...
                 <h3 className="font-bold text-white text-sm">Outras Configurações</h3>
              </div>

              <div className="p-6 flex flex-wrap items-end gap-x-10 gap-y-6">
                <div className="space-y-2">
                   <label htmlFor="system-type" className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-0.5">
                     Modo do Sistema
                   </label>
                   <div id="system-type" className="flex p-1 bg-slate-100 rounded-xl w-fit">
                      {['Mesa', 'Comanda'].map((type) => (
                        <button
                          key={type}
                          type="button"
                          onClick={() => setValue('systemType', type as any)}
                ...
                <div className="space-y-2">
                   <label htmlFor="quantity" className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-0.5">
                     Disponibilidade
                   </label>
                   <div className="flex items-center gap-2">
                      <Input.Root error={!!errors.quantity} className="w-24 bg-white border-slate-200 h-9 rounded-xl">
                         <Input.Control 
                           id="quantity"
                           type="number" 
                ...
                <div className="space-y-2">
                   <label htmlFor="service-fee" className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-0.5">
                     Taxa de Serviço (%)
                   </label>
                   <Input.Root error={!!errors.serviceFee} className="w-24 bg-white border-slate-200 h-9 rounded-xl">
                       <Input.Control 
                           id="service-fee"
                           type="number" 
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-6 overflow-hidden">
            ...
            <div className="bg-orange-500 rounded-3xl p-6 text-white shadow-xl shadow-orange-500/10 flex flex-col gap-4 relative overflow-hidden">
                ...
                <div className="relative">
                  <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-80 mb-1">Assinatura Ativa</p>
                  <h3 className="text-xl font-bold italic tracking-tight">Edição Premium</h3>
                </div>
                
                <div className="bg-white/10 rounded-2xl p-4 flex items-center justify-between">
                   <div className="space-y-0.5">
                      <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Validade</p>
                      <p className="font-bold text-sm">12 Out 2026</p>
                   </div>
                   <Button size="sm" className="bg-white text-orange-600 hover:bg-orange-50 font-bold border-none h-8 px-4 text-[10px] shadow-sm">
                      RENOVAR
                   </Button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col gap-4 shadow-sm">
                <div className="flex items-center gap-2 mb-1">
                   ...
                   <h3 className="font-bold text-slate-800 text-sm tracking-tight">Gerenciamento de Backup</h3>
                </div>
                
                <div className="space-y-3">
                  <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl">
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Caminho Atual</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-1">
                    <Button variant="outline" className="h-9 rounded-xl text-[10px] font-bold border-slate-200">
                       ALTERAR
                    </Button>
                    <Button variant="outline" className="h-9 rounded-xl text-[10px] font-bold border-slate-200 gap-2">
            ...
            <div className="mt-auto pb-4 px-2 space-y-2">
               <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-300">Suporte Técnico</p>
               <button type="button" className="text-xs font-bold text-slate-500 hover:text-orange-500 transition-colors flex items-center gap-2">
