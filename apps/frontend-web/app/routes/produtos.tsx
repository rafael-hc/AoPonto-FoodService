...
export default function ProdutosPage() {
  ...
  const editMutation = useProductsControllerEdit({
    mutation: {
      onSuccess: () => refetch()
    }
  })
  ...
  const handleDelete = (id: string) => {
    if (confirm('Deseja realmente arquivar este produto?')) {
      alert('Endpoint de deleção /products/:id em desenvolvimento.')
    }
  }
      ...
      <Card.Root className="overflow-hidden border border-slate-200 shadow-xl shadow-slate-200/40 rounded-2xl bg-white">
        ...
        {!isLoading && filteredProducts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
            <div className="w-20 h-20 bg-orange-50 text-orange-200 rounded-[2rem] flex items-center justify-center mb-6 transform rotate-6 scale-110 shadow-inner">
